...
namespace VoxelValley.Enviroment.RegionManagement.Regions
{
    public static class GenerationPipeline
    {
        ...
        {
            Biome[,] biomes = new Biome[CommonConstants.World.ChunkSizeXZ, CommonConstants.World.ChunkSizeXZ];
            ushort[,] heights = new ushort[CommonConstants.World.ChunkSizeXZ, CommonConstants.World.ChunkSizeXZ];
        ...
        {
            for (ushort x = 0; x < CommonConstants.World.ChunkSizeXZ; x++)
                for (ushort z = 0; z < CommonConstants.World.ChunkSizeXZ; z++)
        ...
        static void InterpolateBiomes(int worldBaseX, int worldBaseZ, ref Biome[,] biomes, ref ushort[,] heights, ref ushort[,,] voxels)
        {
            ...
            for (ushort x = 0; x < CommonConstants.World.ChunkSizeXZ; x++)
                for (ushort z = 0; z < CommonConstants.World.ChunkSizeXZ; z++)
                    if (x - interpolationLengthHalfed > 0 && x + interpolationLengthHalfed < CommonConstants.World.ChunkSizeXZ &&
                        z - interpolationLengthHalfed > 0 && z + interpolationLengthHalfed < CommonConstants.World.ChunkSizeXZ &&
        ...
        {
            for (ushort x = 0; x < CommonConstants.World.ChunkSizeXZ; x++)
                for (ushort y = 0; y < CommonConstants.World.ChunkSizeY; y++)
                    for (ushort z = 0; z < CommonConstants.World.ChunkSizeXZ; z++)
